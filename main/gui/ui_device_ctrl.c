...
static void ui_dev_ctrl_page_func_click_cb(lv_event_t *e)
{
    ...
    else if (UI_DEV_AIR == g_active_dev_type)
    {
        // bool state = app_air_get_state();
        // app_air_set_power(!state);
        ESP_LOGI(TAG, "Air");
    }
    else
    {
...
void ui_device_ctrl_start(void (*fn)(void))
{
    ...
    for (size_t i = 0; i < 4; i++)
    {
        ...
        if (UI_DEV_LIGHT == i)
        {
            ui_dev_ctrl_set_state(i, true /*app_pwm_led_get_state()*/);
        }
        else if (UI_DEV_SWITCH == i)
        {
            ui_dev_ctrl_set_state(i, true /*app_switch_get_state()*/);
        }
        else if (UI_DEV_FAN == i)
        {
            ui_dev_ctrl_set_state(i, true /*app_fan_get_state()*/);
        }
        else if (UI_DEV_AIR == i)
        {
            ui_dev_ctrl_set_state(i, true /*app_air_get_state()*/);
        }
